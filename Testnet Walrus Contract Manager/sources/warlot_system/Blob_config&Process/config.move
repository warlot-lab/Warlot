...
public(package) fun reduce_cycle_limit(blob_cfg: &mut BlobConfig): u64{
     if(option::is_none(&blob_cfg.cycle_limit)){
        return  0
    };
    let cycle =  option::borrow_mut(&mut blob_cfg.cycle_limit);
    *cycle = *cycle + 1;
    *cycle
}

// // get Id of the blob object
// public(package) fun get_blob_obj_id(blob_cfg: &BlobConfig):ID{
...
// public(package) fun blob_current(blob_cfg: &BlobConfig): u32{
...
// public(package) fun get_renew_epoch_count(blob_cfg: &BlobConfig, system: &System, ahead: u32): u32{
