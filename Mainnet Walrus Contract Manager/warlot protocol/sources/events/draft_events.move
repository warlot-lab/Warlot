...
public struct DraftPinned has copy, drop, store {
    ...
    draft_id: ID,
    draft_index: u64,
    /// The pass or admin capability the write was made under.
    credential: ID,
    /// Which of the two `credential` names: `0` a writer pass, `1` an operator's
    /// admin capability.
    ///
    /// The draft itself records neither. An id alone cannot be resolved to a type
    /// by anything on chain, and a stored struct cannot gain a discriminant later,
