...
func (db *DB) InsertNewUser(data map[string]interface{}) error {
	...
	log.Println("✅ NewUser written to DB for\n\n", userAddr)
	//Commit
	return tx.Commit()
}
...
func (db *DB) InsertManagedBlob(data map[string]interface{}) error {
	...
	EncodedSizeStr, ok := data["encoded_size"].(string)
	if !ok {
		return fmt.Errorf("InsertPublisherBlob: missing EncodedSizeStr")
	}

	EncodedSize, err := strconv.ParseInt(EncodedSizeStr, 10, 64)
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec(
		...
		currentEpoch, epochSet, cycleEnd, EncodedSize,
	)
	if err != nil {
		return err
	}
...
func (db *DB) InsertPublisherBlob(data map[string]interface{}) error {
	...
	_, err = tx.Exec(
		`INSERT INTO poc.warlot_stored_file(
             ...
             (SELECT registry_id FROM poc.registry WHERE user_id=(SELECT user_id FROM poc.app_user WHERE address=$1)),
             $2, $3, $4, $5, $6,
             (SELECT origin_id FROM poc.blob_origin WHERE name='publisher'), $7
         )
         ON CONFLICT (blob_obj_id) DO UPDATE SET
             size_bytes    = EXCLUDED.size_bytes,
             current_epoch = EXCLUDED.current_epoch,
	...
	log.Println("✅ WarlotFileStore written to DB for blob\n\n", blobID)
	return tx.Commit()
}
...
func (db *DB) UpsertBlobAttributes(data map[string]interface{}) error {
	ownerAddr, ok := data["owner"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing owner")
	}
	blobID, ok := data["blob_obj_id"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing blob_obj_id")
	}
	projectName, ok := data["project_name"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing project_name")
	}
	bucketName, ok := data["bucket_name"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing bucket_name")
	}
	baseFileName, ok := data["file_name"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing file_name")
	}
	fileType, ok := data["file_type"].(string)
	if !ok {
		return fmt.Errorf("UpsertBlobAttributes: missing file_type")
	}

	fullFileName := fmt.Sprintf("%s.%s", baseFileName, fileType)

	log.Printf(
		"📦 Upserting blob attributes: owner=%s blob=%s project=%s bucket=%s file=%s\n",
		ownerAddr, blobID, projectName, bucketName, fullFileName,
	)
	...
	_, err = tx.Exec(
		`UPDATE poc.warlot_stored_file
		 SET project_name = $1,
			 ...
			 file_name = $3
		 WHERE blob_obj_id = $4`,
		projectName, bucketName, fullFileName, blobID,
	)
	...
	log.Printf("✅ Blob attributes upserted for blob %s\n", blobID)
	return tx.Commit()
}
